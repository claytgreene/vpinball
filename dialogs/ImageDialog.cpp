...
{ 
   ...
   if (ptCur)
   {
    ...
    //Below code block will fill a vector with all the images in Image Manager and the table objects that reference them
    //MessageBox("Show Where Used", "Show Where Used", MB_OK); 
    //CCO(PinTable) *const pt = g_pvp->GetActiveTable();
    //pt->ShowWhereImagesUsed();
...
void ImageDialog::LoadPosition()
{
   ...
   const int w = g_pvp->m_settings.LoadValueWithDefault(Settings::Editor, "ImageMngWidth"s, 800);
   const int h = g_pvp->m_settings.LoadValueWithDefault(Settings::Editor, "ImageMngHeight"s, 600);
   POINT p {x, y};
...
//-------------------- Below is the 'WhereUsedDialog' dialog class methods -------------------------------------------------------------------------------
...
INT_PTR WhereUsedDialog::DialogProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
    {
      ...
      //Remmed below block because we don't have any controls other than the listbox
      //m_resizer.AddChild(GetDlgItem(IDC_PICTUREPREVIEW).GetHwnd(), CResizer::topright, 0);
      //m_resizer.AddChild(GetDlgItem(IDC_IMPORT).GetHwnd(), CResizer::topright, 0);
      //m_resizer.AddChild(GetDlgItem(IDC_REIMPORT).GetHwnd(), CResizer::topright, 0);
      //m_resizer.AddChild(GetDlgItem(IDC_REIMPORTFROM).GetHwnd(), CResizer::topright, 0);
      ...
      // Create a listvew then fill it with the items in the vector 'vWhereUsed'
      //      
      ...
      ////Create Listview Item 0 Subitem 0
      //LVITEM lvitem;
      //lvitem.mask = LVIF_DI_SETITEM | LVIF_TEXT | LVIF_PARAM;
      //lvitem.iItem = 0;
      //lvitem.iSubItem = 0;
      //lvitem.pszText = (LPSTR)"Source Image 1";
      //const int index = ListView_InsertItem(hListView, &lvitem);
      ////Create Listview Item 0 Subitem 1
      //ListView_SetItemText(hListView, index, 1, (LPSTR) "In Use By Object 1");

      ////Create Listview Item 1 Subitem 0
      //lvitem.mask = LVIF_DI_SETITEM | LVIF_TEXT | LVIF_PARAM;
      //lvitem.iItem = 1;
      //lvitem.iSubItem = 0;
      //lvitem.pszText = (LPSTR) "Source Image 2";
      //const int indexi = ListView_InsertItem(hListView, &lvitem);
      ////Create Listview Item 1 Subitem 1
      //ListView_SetItemText(hListView, indexi, 1, (LPSTR) "In Use By Object 2");
...
//void WhereUsedDialog::UpdateImages()
//{
//   const HWND hSoundList = GetDlgItem(IDC_SOUNDLIST).GetHwnd();
//   const int count = ListView_GetSelectedCount(hSoundList);
//
    ...
    if (selectedItemsCount) // if some items are selected???
    {
      ...
      if (sel != -1)
      {
         ...
         if (ppi != nullptr)
         {
            ...
            ofn.lpstrDefExt = ext2.c_str() + idx2 + 1;
            // check which default file extension should be selected
            ofn.lpstrFilter = "PNG (.png)\0*.png;\0Bitmap (.bmp)\0*.bmp;\0JPEG (.jpg/.jpeg)\0*.jpg;*.jpeg;\0IFF (.iff)\0*.IFF;\0PCX (.pcx)\0*.PCX;\0PICT (.pict)\0*.PICT;\0Photoshop "
                              "(.psd)\0*.psd;\0TGA (.tga)\0*.tga;\0TIFF (.tiff/.tif)\0*.tiff;*.tif;\0WEBP (.webp)\0*.webp;\0EXR (.exr)\0*.exr;\0HDR (.hdr)\0*.hdr\0";
            if (!lstrcmpi(ofn.lpstrDefExt, "png"))
               ofn.nFilterIndex = 1;
            else if (!lstrcmpi(ofn.lpstrDefExt, "bmp"))
               ofn.nFilterIndex = 2;
...
//void WhereUsedDialog::DeleteImage()
//{
//   const HWND hSoundList = GetDlgItem(IDC_SOUNDLIST).GetHwnd();
//   const int count = ListView_GetSelectedCount(hSoundList);
//
...
void WhereUsedDialog::UpdateAll()
{
    const HWND hSoundList = GetDlgItem(IDC_SOUNDLIST).GetHwnd();
    const int count = ListView_GetSelectedCount(hSoundList);
    bool errorOccurred = false;
...
void WhereUsedDialog::ReimportFrom()
{
    const HWND hSoundList = GetDlgItem(IDC_SOUNDLIST).GetHwnd();
    const int sel = ListView_GetNextItem(hSoundList, -1, LVNI_SELECTED);
